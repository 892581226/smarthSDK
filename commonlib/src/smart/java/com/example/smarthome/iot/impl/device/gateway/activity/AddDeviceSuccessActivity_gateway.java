...
import com.example.smarthome.iot.impl.device.gateway.bean.UpdateGateWay;
 ...
 * version:
 */
public class AddDeviceSuccessActivity_gateway extends BaseActivity implements View.OnClickListener, BaseQuickAdapter.OnItemChildClickListener {
...
//        mDeviceInfoBean = getIntent().getParcelableExtra("DeviceInfoBean");
        ...
        if (intent!=null){
            datas = (GateWayBeanVo.GateWayBeanVoData) intent.getSerializableExtra("datas");
        }
        if(datas!=null){
            mDeviceName.setText(datas.getDeviceName());
        }
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.top_back){
            ...
            if (datas != null && checkedRoom != null && !StringUtils.isEmpty(mDeviceName.getText().toString().trim())) {
                ...
                loadingDialog.show();
                String getewayId = SPUtils.get(this, "gateWayId_sun", "");
                smartUpdateDevice(getewayId, mDeviceName.getText().toString().trim(), checkedRoom.getRoomName(), checkedRoom.getRoomId());
            } else {
                ToastUtil.show(AddDeviceSuccessActivity_gateway.this,"设备名称或设备位置为空");
            }
        }
    }
...
//        obj.put("roomId", roomId);
        ...
        UpdateGateWay obj=new UpdateGateWay();
        obj.setGatewayId(deviceId);
        obj.setGatewayName(deviceName);
        obj.setLocation(location);
        obj.setRoomId(roomId);

        Log.e("设备参数",deviceId+" "+deviceName+" "+location+" "+roomId);
        OkGo.<String>post(Constant.HOST+Constant.Device_updateGateWay)
...
//                        addDisposable(d);
                    ...
                    public void onNext(CommonResp resp) {
                        LogUtils.e("onNext","   =---===="+resp.getErrorCode()+" "+resp.getMessage());
                        if(resp !=null){
...
//                        dismissLoading();
    ...
    Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 200) {
                ToastUtil.show(AddDeviceSuccessActivity_gateway.this,"操作成功");
                EventBus.getDefault().post(new UpdateFamilyEvent(true));
                destoryActivity("AppendGatewayActivity");
                destoryActivity("AppendGatewayActivity_two");
                destoryActivity("GatewayNetSettingActivity_gateway");
                destoryActivity("ScanAddDeviceActivity");
                finish();
            } else if (msg.what == 202) {
                ToastUtil.show(AddDeviceSuccessActivity_gateway.this,"操作失败");
            }
        }
    };
