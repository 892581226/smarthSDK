...
import com.example.smarthome.iot.entry.SmartDevNumb;
 ...
 * version:
 */
public class DeviceListAdapter2 extends BaseQuickAdapter<SmartInfoVo.FamilysBean.DeviceInfoBean, BaseViewHolder> {
    ...
    public DeviceListAdapter2(List<SmartInfoVo.FamilysBean.DeviceInfoBean> data) {
...
//            helper.setText(R.id.item_iot_device_name, item.getDeviceName());
            if(isMydevice){
                ...
                getDeviceNub(item.getId(),helper);
               /* if(item.getDeviceType().equalsIgnoreCase(DeviceType.HILINK_WIRLESS)){

                }else if(item.getDeviceType().equalsIgnoreCase(DeviceType.HILINK_WIRED)){
                    getDeviceNub(item.getId(),helper);
                }*/
...
//            helper.setImageResource(R.id.item_iot_device_img, icon);
    ...
    private void getDeviceNub(String gatewayId, BaseViewHolder helper) {

        OkGo.<String>get(Constant.HOST+Constant.Gateway_getDevice)
...
//                        addDisposable(d);
                    ...
                    public void onNext(CommonResp resp) {
                        if (resp.getErrorCode().equalsIgnoreCase("200")){
                            SmartDevNumb smartDevNumb = JSON.parseObject(resp.getResult(), SmartDevNumb.class);
                            List<SmartDevNumb.ListNub> list = smartDevNumb.getList();
                            helper.setText(R.id.item_iot_device_address,"关联"+list.size()+"个设备");
                        }
