...
import com.xhwl.commonlib.uiutils.popmenu.DropPopLayoutIot;
import com.xhwl.commonlib.uiutils.popmenu.TriangleIndicatorView;
...
public class DropPopMenuIot {
    ...
    private DropPopLayoutIot mDropPopLayout;

    private ListView mListView;
    private View mBtnView;
    private DropPopMenuAdapter mDropPopMenuAdapter;
    private DropPopStringMenuAdapter mDropPopStringMenuAdapter;
    private OnItemClickListener mOnItemClickListener;

    private List<SmartInfoList.FamilysBean> mMenuItemList;
    ...
    public DropPopMenuIot(Context context, boolean isLeft) {
        mContext = context;
        init();

        mDropPopLayout = new DropPopLayoutIot(context, isLeft);
    ...
    private void create() {
        ...
        mDropPopLayout.setOnTouchListener((view, motionEvent) -> {
    ...
    private void initListView() {
        ...
        mListView.setOnItemClickListener((adapterView, view, position, id) -> {
            if (mOnItemClickListener != null) {
                SmartInfoList.FamilysBean menuItem = null;
    ...
    public void setMenuList(List<SmartInfoList.FamilysBean> menuList) {
        if (mMenuItemList != null) {
            mMenuItemList.clear();
        } else {
    ...
    public void setMenu(List<String> menuList) {
        if (mStrings != null) {
            mStrings.clear();
        } else {
            mStrings = new ArrayList<>();
        }
        mStrings.addAll(menuList);

        checkWidth();
        checkHeight();

        initListView();
        if (mDropPopStringMenuAdapter == null) {
            mDropPopStringMenuAdapter = new DropPopStringMenuAdapter();
            mListView.setAdapter(mDropPopStringMenuAdapter);
        } else {
            mDropPopStringMenuAdapter.notifyDataSetChanged();
        }
    }
    ...
    private void checkWidth(List<SmartInfoList.FamilysBean> familyListItemVos) {
        int size = getListSize(familyListItemVos);
        String temp = "";
        for (int i = 0; i < size; i++) {
            String itemTitle = familyListItemVos.get(i).getFamilyName();
    ...
    private void checkWidth() {
        int size = getListSize(mStrings);
        String temp = "";
        for (int i = 0; i < size; i++) {
            String itemTitle = mStrings.get(i);
            if (itemTitle.length() > temp.length()) {
                temp = itemTitle;
            }
        }
    ...
    public void show(Activity mContent, final View parent) {
        mBtnView = parent;
        setBackgroundAlpha(mContent,0.3f);
        ...
        int y = location[1];
        int popMenuHeight = mTotalHeight + mTriangleUpIndicatorView.getRealHeight()+30;
    ...
    private void updateViewPosition(View parent) {
        ...
        if (leftMargin < rightMargin) {//在左侧
            ...
            if (rightMargin >= containerViewHalfWidth) {
                ...
                if (upIndicatorParams.leftMargin < containerParams.leftMargin + mIndicatorToContainerMinMargin) {//矫正箭头在列表左边
                    ...
                    upIndicatorParams.rightMargin = 70;
                }
            }
        } else {//在中间
        ...
        mDropPopLayout.setOrientation(false);
    ...
    private class DropPopMenuAdapter extends BaseAdapter {
        ...
        @Override
        public SmartInfoList.FamilysBean getItem(int position) {
        ...
        public View getView(final int position, View view, ViewGroup viewGroup) {
            ...
            holder.textTv.setText(getItem(position).getFamilyName());
            holder.textTv.setGravity(Gravity.CENTER);
            if (mItemTextColor != 0) {
            ...
            int i= (int) (mScreenWidth*0.255)>mWidth?(int)(mScreenWidth*0.255):mWidth;
            if (i<=700){
                mWidth = (int) (mScreenWidth*0.255)>mWidth?(int)(mScreenWidth*0.255):mWidth;
            }else {
                mWidth=700;
            }
            return view;
    ...
    private class DropPopStringMenuAdapter extends BaseAdapter {
    ...
    public interface OnItemClickListener {
        void onItemClick(AdapterView<?> adapterView, View view, int position, long id, SmartInfoList.FamilysBean menuItem);
    }
