...
public class SecondMatchActivity extends BaseActivity implements View.OnClickListener, YaokanSDKListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        showDlg();
        Yaokan.instance().getMatchingResult(YaoKanSDK.curTid, YaoKanSDK.curBid, YaoKanSDK.curGid);
    }
        ...
        if (YaoKanSDK.curTid == 7) {
            ...
            gv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    Yaokan.instance().sendCmd(YaoKanSDK.curDid, rc.getRid(), key, rc.getBe_rc_type(), null, null);
                }
            });
        }
    }
    ...
    private void sendCmd() {
        Yaokan.instance().sendAirCmd(YaoKanSDK.curDid, rc.getRid(), new AirOrder(curMode, curSpeed, curTemp, curVer, curHor));
        setText();
    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_pre) {
            ...
            rc.setName(YaoKanSDK.curBName + YaoKanSDK.curTName + " " + rc.getRmodel());
            rc.setMac(YaoKanSDK.curMac);
            if (rc.getBe_rc_type() == 1 && YaoKanSDK.operators != null) {//机顶盒有运营商数据
                rc.setProvider(YaoKanSDK.operators.getJson());
                YaoKanSDK.operators = null;
            }
            Yaokan.instance().downloadCodeToDevice(YaoKanSDK.curDid, rc.getRid(), rc.getBe_rc_type());
        } else if (id == R.id.btn_get_again) {
            showDlg();
            match();
        ...
        } else if (id == R.id.btn_on) {
            Yaokan.instance().sendCmd(YaoKanSDK.curDid, rc.getRid(), "on", rc.getBe_rc_type(), null, null);
        } else if (id == R.id.btn_off) {
            Yaokan.instance().sendCmd(YaoKanSDK.curDid, rc.getRid(), "off", rc.getBe_rc_type(), null, null);
            ...
            if (rc.getAirCmd().getAttributes().getVerticalIndependent() == 1) {
                ...
                Yaokan.instance().sendAirCmd(YaoKanSDK.curDid, rc.getRid(), Swing.Ver, isOpen);
        ...
        } else if (id == R.id.btn_hor) {
            if (rc.getAirCmd().getAttributes().getVerticalIndependent() == 1) {
                horIndex++;
                boolean isOpen = (horIndex % 2 == 1);
                Yaokan.instance().sendAirCmd(YaoKanSDK.curDid, rc.getRid(), Swing.Hor, isOpen);
    ...
    private void match() {
        if (mMatch != null) {
            Yaokan.instance().remoteInfo(mMatch.getRid(), YaoKanSDK.curTid);
        } else {
            dismiss();
        }
                ...
                if (!isFinishing()) {
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (result != null && !TextUtils.isEmpty(YaoKanSDK.curMac) && YaoKanSDK.curMac.equals(result.getMac())) {
                                String msg = "";
