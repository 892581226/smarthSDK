...
public class PageIndicatorView extends LinearLayout {
    ...
    private void init(Context context) {
        ...
        dotSize = UiTools.dip2px(context, dotSize);
        margins = UiTools.dip2px(context, margins);
    }
    ...
    public void initIndicator(int count) {
        ...
        if (indicatorViews.size() > 0) {
            indicatorViews.get(0).setBackgroundResource(R.drawable.dot_2);
        }
    }
    ...
    public void setSelectedPage(int selected) {
        if(indicatorViews != null && indicatorViews.size() > 0){
            for (int i = 0; i < indicatorViews.size(); i++) {
                if (i == selected) {
                    indicatorViews.get(i).setBackgroundResource(R.drawable.dot_2);
                } else {
                    indicatorViews.get(i).setBackgroundResource(R.drawable.dot_1);
                }
            }
        }
    }
